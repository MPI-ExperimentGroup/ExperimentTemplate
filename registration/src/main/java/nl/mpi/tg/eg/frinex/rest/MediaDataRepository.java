...
public interface MediaDataRepository extends PagingAndSortingRepository<MediaData, Long>, MediaDataStreamer {
    ...
    @Query("SELECT new MediaData(a.id, a.submitDate, a.experimentName, a.screenName, a.userId, a.stimulusId, a.recordingFormat, a.downloadPermittedWindowMs, a.mediaUUID, a.partNumber) FROM MediaData a WHERE "
...
//    List<MediaData> findAllBySubmitDateBetween(Date submitDateStart, Date submitDateEnd);
    @Query("SELECT new MediaData(a.id, a.submitDate, a.experimentName, a.screenName, a.userId, a.stimulusId, a.recordingFormat, a.downloadPermittedWindowMs, a.mediaUUID, a.partNumber) FROM MediaData a WHERE a.partNumber = 0 AND a.submitDate BETWEEN :start AND :end")
    List<MediaData> findBySubmitDateBetween(@Param("start") Date start, @Param("end") Date end);

    @Query("SELECT new MediaData(a.id, a.submitDate, a.experimentName, a.screenName, a.userId, a.stimulusId, a.recordingFormat, a.downloadPermittedWindowMs, a.mediaUUID, a.partNumber) FROM MediaData a WHERE a.userId = :userId AND a.partNumber = 0 ORDER BY a.submitDate ASC")
    @Transactional
...
//    public List<MediaData> findBySubmitDateOrderBySubmitDateAsc(@Param("submitDate") String userId);
    ...
    @Transactional
    @Query("SELECT new MediaData(a.id, a.submitDate, a.experimentName, a.screenName, a.userId, a.stimulusId, a.recordingFormat, a.downloadPermittedWindowMs, a.mediaUUID, a.partNumber) FROM MediaData a WHERE a.shortLivedToken = :shortLivedToken AND a.userId = :userId AND a.partNumber = 0")
