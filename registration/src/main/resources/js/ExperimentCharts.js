...
function loadMore(tableId) {
    ...
    $.getJSON(dataUrl + ((parametersString.length == 0) ? "?" : "&") + 'page=' + pageNumber + '&sort=' + sortColumn + ',' + sortDirection , function (responseData) {
        ...
        for (const recordData of responseData._embedded[Object.keys(responseData._embedded)[0]]) {
            ...
            for (const columnHeader of $("#" + tableId + " thead tr th")) {
                ...
                if (columnId === "mediaFile") {
                    ...
                    dataRow += "<td>";
                    dataRow += "<a class=\"downloadLink\" href=\"" + recordData.mediaPath + "\"><i class=\"fa fa-download\"></i></a>";
                    dataRow += "</td>";
                    ...
                    dataRow += "</td>";
                } else if (columnId === "tagDate" || columnId === "submitDate" || columnId === "viewDate") {
                    const tagDate = moment(recordData[columnId]);
                    dataRow += "<td>" + tagDate.format("YYYY-MM-DD HH:mm:ss.SSS") + "<div class='balloontext'>" + recordData[columnId] + "</div></td>";
                } else if (columnId.length > 0) {
                    dataRow += "<td>" + ((recordData[columnId] !== null) ? recordData[columnId] : "<div class='balloontext'>" + recordData[columnId] + "</div>") + "</td>";
                }
            }
            dataRow += "</tr>";
        ...
        if (typeof setColumnWidths === "function") setColumnWidths();
    });
}
