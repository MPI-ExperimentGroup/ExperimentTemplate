...
//        experiment.getPresenterScreen().add(addVideoFailedPage(presenterFeatureRepository));
    ...
    private void addDobesStimuli(final Experiment experiment) {
        ...
        tagSet.add("videotag");
        for (int i = 0; i < 10; i++) {
            final Stimulus stimulus = new Stimulus(null, null, null, "videotag" + i + ".png", "videotag" + i, "videotag" + i, 0, tagSet);
        ...
        for (String tag : new String[]{"sim", "mid", "diff", "noise"}) {
            for (String label : new String[]{"rabbit", "cat", "muffin", "you"}) {
                tagSet.clear();
                tagSet.add(tag);
                stimuliList.add(new Stimulus(tag + "_" + label, tag + "_" + label, tag + "_" + label, tag + "_" + label + ".jpg", tag + "_" + label, tag + "_" + label, 0, tagSet));
            }
        }
        tagSet.clear();
        for (String word : "termites scorpions centipedes".split(" ")) {
            for (String speaker : "Rocket Festival Thai ประเพณีบุญบั้งไฟ Lao ບຸນບັ້ງໄຟ".split(" ")) {
                for (int i = 0; i < 6; i++) {
                    stimuliList.add(new Stimulus(word + "_" + speaker + "_" + i + ".mp3", word + "_" + speaker + "_" + i + ".mp4", word + "_" + speaker + "_" + i + ".ogg", word + "_" + speaker + "_" + i + ".jpg", word + "_" + speaker + "_" + i, word + "_" + speaker + "_" + i, 0, new HashSet<>(Arrays.asList(new String[]{word, speaker}))));
                }
            }
        }
    ...
    private void addStimuli(final Experiment experiment) {
        ...
        stimuliList.add(new Stimulus("one.mp3", "one.mp4", "one.ogg", "one.jpg", "One", "One", 0, tagSet));
        tagSet.add("multiple words");
        stimuliList.add(new Stimulus("two.mp3", "two.mp4", "two.ogg", "two.jpg", "Two", "Two", 0, tagSet));
        tagSet.clear();
        tagSet.add("FILLER_AUDIO");
        stimuliList.add(new Stimulus("three.mp3", "three.mp4", "three.ogg", "three.jpg", "Three", "Three", 0, tagSet));
        stimuliList.add(new Stimulus("four.mp3", "four.mp4", "four.ogg", "four.jpg", "Four", "Four", 0, tagSet));
        tagSet.clear();
        tagSet.add("NOISE_AUDIO");
        stimuliList.add(new Stimulus("five.mp3", "five.mp4", "five.ogg", "five.jpg", "Five", "Five", 0, tagSet));
        stimuliList.add(new Stimulus("six.mp3", "six.mp4", "six.ogg", "six.jpg", "Six", "Six", 0, tagSet));
        for (String tag : new String[]{"sim", "mid", "diff", "noise"}) {
            for (String label : new String[]{"rabbit", "cat", "muffin", "you"}) {
                tagSet.clear();
                tagSet.add(tag);
