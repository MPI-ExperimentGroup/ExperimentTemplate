...
//        experimentRepository.save(new SentenceCompletion(new Joost02()).getExperiment());
        ...
        for (Experiment experiment : experimentRepository.findAll()) {
            eventRepository.save(new PublishEvents(experiment, new Date(), new Date(), PublishEvents.PublishState.validate, true, false, false, false, false, false, null, null, null));
        }
    }
    ...
    public Experiment getAllOptionsExperiment(MetadataRepository metadataRepository, PresenterFeatureRepository presenterFeatureRepository, PresenterScreenRepository presenterScreenRepository) {
        ...
        if (metadataRepository != null) {
            metadataRepository.saveAll(experiment.getMetadata());
        }
        final Calendar datePublished = new GregorianCalendar(2020, 02, 02);
        final Calendar dateExpired = new GregorianCalendar(2020, 02, 20);
        experiment.getPublishEvents().add(new PublishEvents(experiment, datePublished.getTime(), dateExpired.getTime(), PublishEvents.PublishState.validate, true, true, true, true, true, true, null, null, null));
...
//        experiment.getPresenterScreen().add(addVideoFailedPage(presenterFeatureRepository));
        ...
        if (presenterFeatureRepository != null) {
            presenterFeatureRepository.saveAll(aboutScreen.getPresenterFeatureList());
        }
        if (presenterFeatureRepository != null) {
            presenterFeatureRepository.saveAll(autoMenu.getPresenterFeatureList());
        }
        ...
        if (presenterFeatureRepository != null) {
            presenterScreenRepository.saveAll(experiment.getPresenterScreen());
        }
        return experiment;
    }
...
//            maxScreenAddCount--;
            ...
            if (presenterFeatureRepository != null) {
                presenterFeatureRepository.saveAll(presenterScreen.getPresenterFeatureList());
            }
...
//                        case maxStimuliPerTag:
                            case minimum:
                                ...
                                break;
                            case initial:
                                presenterFeature.addFeatureAttributes(attribute, "35");
                                break;
                            case offset:
                            case dataChannel:
...
//                                break;
        ...
        switch (featureType.getRequiresChildType()) {
            case hasTrueFalseCondition:
                ...
                break;
            case hasTrueFalseErrorCondition:
                presenterFeature.getPresenterFeatureList().add(addFeature(experiment, presenterType, FeatureType.conditionTrue, presenterFeatureRepository, addOptionalAttributes));
                presenterFeature.getPresenterFeatureList().add(addFeature(experiment, presenterType, FeatureType.conditionFalse, presenterFeatureRepository, addOptionalAttributes));
