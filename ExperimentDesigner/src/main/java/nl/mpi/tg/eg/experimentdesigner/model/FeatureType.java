...
public enum FeatureType {
    ...
    serverValueAssign(false, new FeatureAttribute[]{targetOptions, fieldName}, "Assigns a value from targetOptions based on the distribution data known by the server and puts the result into the metadata field identified by fieldName. Any value in the metadata field will be overwritten when valueOptions is supplied. However if the metadata field contains an item from targetOptions it will be unchanged and the server assigned counts will not be affected.", Contitionals.hasErrorSuccess, Contitionals.none),
