...
public enum FeatureType {
    ...
    clearTimer(false, false, new FeatureAttribute[]{listenerId}, "Clears the start time of a persistent timer matching listenerId. A subsequent startTimer with the same listenerId can be used to assign a new start time."),
    ...
    triggerDefinition(true, false, new FeatureAttribute[]{listenerId, threshold, maximum}, "The contents of this element will be activated when matched by the listenerId attribute of trigger or triggerRandom (for example), providing the number of trigger occurrences has reached the threshold and within the maximum. A triggerDefinition can have more than one stimulus available (eg when it is in an eachStimulus or hasMoreStimulus). If this is the case then stimulus where triggerDefinition is defined will be used. If there is no stimulus where the triggerDefinition is defined then the stimulus from the point at which it is triggered will be used."),
