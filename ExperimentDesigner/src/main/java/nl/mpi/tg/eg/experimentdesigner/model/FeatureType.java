...
public enum FeatureType {
    ...
    actionTokenButton(true, new FeatureAttribute[]{hotKey, styleName, groupId}, "Creates a button where any tokens in its label, styleName and groupId are evaluated before displaying. When clicked evaluates the contents of its element.", Contitionals.any, Contitionals.hasActionButtons, Contitionals.none),
    ...
    ratingFooterButton(false, new FeatureAttribute[]{eventTag, dataChannel, ratingLabels, ratingLabelLeft, ratingLabelRight, styleName, groupId}, "Adds rating buttons in the footer based on the ratingLabels provided. If the rating label contains the numbers 0 to 9 then those number keys will be available as hotkeys. If there are exactly two rating labels then the keys z and . will be available as hotkeys. If groupId contains tokens they will be replaced with the respective values.", Contitionals.any, Contitionals.stimulusAction),
    ...
    serverValueAssign(false, new FeatureAttribute[]{targetOptions, fieldName}, "Assigns a value from targetOptions based on the distribution data known by the server for the given presenter and puts the result into the metadata field identified by fieldName. Any value in the metadata field will be overwritten when valueOptions is supplied. However if the metadata field contains an item from targetOptions it will be unchanged and the server assigned counts will not be affected.", Contitionals.hasErrorSuccess, Contitionals.none),
    serverValueComplete(false, new FeatureAttribute[]{fieldName}, "Marks the server assigned value stored in the metadata field identified by fieldName as completed on the server. This will not change the value stored in the metadata field. If a new assignment is required then the metadata field would need to be manually cleared before requesting a new value via serverValueAssign. The completion of values will affect the distribution and therefore which values might be assigned next.", Contitionals.hasErrorSuccess, Contitionals.none),
    //    @Deprecated //???
    ...
    prevStimulusButton(true, new FeatureAttribute[]{eventTag, repeatIncorrect, hotKey, styleName, groupId}, "Adds a button that when clicked decrements the current stimulus and triggers hasMoreStimulus. This button will not submit a stimulus response on its own. If however there is no previous stimuli then endOfStimulus will be triggered when the button is clicked.", Contitionals.none, Contitionals.stimulusAction),
    nextStimulusButton(true, new FeatureAttribute[]{eventTag, repeatIncorrect, hotKey, styleName, groupId}, "Adds a button that when clicked increments the current stimulus and triggers hasMoreStimulus. This button will not submit a stimulus response on its own. If however there are no more stimuli then endOfStimulus will be triggered when the button is clicked.", Contitionals.none, Contitionals.stimulusAction),
    nextStimulus(false, new FeatureAttribute[]{/*eventTag,*/repeatIncorrect/*, repeatMatching*/}, "Increments the current stimulus and triggers hasMoreStimulus. This will not submit a stimulus response on its own. If however there are no more stimuli then endOfStimulus will be triggered.", Contitionals.none, Contitionals.stimulusAction),
    prevStimulus(false, new FeatureAttribute[]{/*eventTag,*/repeatIncorrect/*, repeatMatching*/}, "Decrements the current stimulus and triggers hasMoreStimulus. This will not submit a stimulus response on its own. If however there is no previous stimulus then endOfStimulus will be triggered.", Contitionals.none, Contitionals.stimulusAction),
    ...
    zeroStimulusStopwatch(false, new FeatureAttribute[]{eventId}, "Each stimulus response can have a number of stimulus time periods recorded. At the point when the time period should start the named period should be zeroed. The stopwatch values are recorded in the stimulus response events for each stimulus based on the eventId attribute.", Contitionals.none, Contitionals.none, Contitionals.stimulusAction, Contitionals.isTimeCritical),
    // zeroStimulusStopwatch or zeroStimulusTimeCounter
    stopStimulusStopwatch(false, new FeatureAttribute[]{eventId}, "Each stimulus response can have a number of stimulus time periods recorded. At the point when the time period should end the named period should be stopped.  The stopwatch values are recorded in the stimulus response events for each stimulus based on the eventId attribute.", Contitionals.none, Contitionals.none, Contitionals.stimulusAction, Contitionals.isTimeCritical),
    // stopStimulusStopwatch or stopStimulusTimeCounter
    ...
    pause(false, new FeatureAttribute[]{msToNext}, "Pause in milliseconds. When the time is up the contents of this element are evaluated. If tokens are required to calculate the ms value, an evaluatePause can be used.", Contitionals.any, Contitionals.none),
    doLater(false, new FeatureAttribute[]{msToNext}, "After the specified milliseconds have passed the contents of this element are evaluated. If tokens are required to calculate the ms value, an evaluatePause can be used.", Contitionals.any, Contitionals.none, Contitionals.isTimeCritical),
    ...
    addRecorderDtmfTrigger(false, new FeatureAttribute[]{dtmf}, "Adds a web recorder event that will trigger when the provided DTMF tone is detected in the recorded audio stream. Only one trigger can be assigned per DTMF code.", Contitionals.isTimeCritical, Contitionals.isRecursiveType, Contitionals.none),
    addRecorderLevelTrigger(false, new FeatureAttribute[]{threshold, thresholdMs, levelIndicatorStyle}, "Adds a web recorder event that will trigger when the a sound in the recorded audio stream exceeds the provided threshold above the ambient noise floor for greater than the provided milliseconds. If the levelIndicatorStyle is set then a recording threshold indicator will be shown. If levelIndicatorStyle is set and a region with the ID of AudioThresholdIndicator exists then that region will be used to position the recording threshold indicator.", Contitionals.isTimeCritical, Contitionals.isRecursiveType, Contitionals.none),
    ...
    regionClear(false, false, new FeatureAttribute[]{regionId}, "If a region matching the regionId exists then it will cleared. Unlike clearPage clearing a region only removes the visual components, so any media that has been placed in the region should be stopped first. If no matching region exists then nothing will be done."),
    ...
    // @TODO: groupMembers could be used to determine the sequence of who goes when and therefore could be changed to groupMembersSequence
    ...
    streamGroupCanvas(false, new FeatureAttribute[]{eventTag, dataChannel, streamChannels, resolution}, "Creates a canvas that is streamed to other members of the group based on the stream communication channels. The stream is terminated when the containing region or page is cleared or when the group network ends.", Contitionals.hasErrorSuccess, Contitionals.groupNetworkAction),
    streamGroupCamera(false, new FeatureAttribute[]{eventTag, dataChannel, streamChannels, resolution}, "Shares a camera stream to other members of the group based on the stream communication channels. The stream is terminated when the containing region or page is cleared or when the group network ends.", Contitionals.hasErrorSuccess, Contitionals.groupNetworkAction),
    // TODO: consider streamGroupMicrophone
    streamRecordStart(false, new FeatureAttribute[]{matchingRegex}, "If a matching group stream exists then its contents will be recorded by the receiver and stored on the server simular to audio recordings. Recordings will be stored in 5-10 second sections. If no matching stream exists then nothing will be done.", Contitionals.hasErrorSuccess, Contitionals.stimulusAction),
    streamRecordStop(false, new FeatureAttribute[]{matchingRegex}, "If a matching group stream exists and is being recorded then that recording will be stopped. If no matching stream exists then nothing will be done.", Contitionals.none, Contitionals.stimulusAction),
    ...
    onValidationError(false, null, false, false, false, Contitionals.any, Contitionals.hasMetadataErrorSuccess),
    onNetworkError(false, null, false, false, false, Contitionals.any, Contitionals.hasMetadataErrorSuccess),
    dataAgreementError(false, null, false, false, false, Contitionals.any, Contitionals.hasMetadataErrorSuccess);
