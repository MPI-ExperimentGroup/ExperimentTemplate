...
public enum FeatureType {
    ...
    captureEnd(false, new FeatureAttribute[]{msToNext}, "When the touch input capture has ended the contents of this element will be evaluated. If msToNext to next is provided then a gap of that ms time is permitted between touches without triggering the captureEnd.", Contitionals.any, Contitionals.touchInputCaptureType),
    ...
    groupNetwork(false, new FeatureAttribute[]{groupMembers, groupCommunicationChannels, /* can we remove groupCommunicationChannels by replacing it with an equivalent of streamChannels eg messageChannels, groupCameraChannels, groupAudioChannels, groupCanvasChannels,*/ phasesPerStimulus}, "Connects a group of participants so that they can interact in a defined manner during the experiment. For each stimulus there is a number of phases which is determined by phasesPerStimulus.", Contitionals.groupNetwork, Contitionals.groupStimulus),
