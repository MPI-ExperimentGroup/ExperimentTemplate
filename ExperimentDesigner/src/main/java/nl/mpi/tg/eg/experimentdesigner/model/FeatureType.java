...
public enum FeatureType {
    ...
    dtmfTone(false, new FeatureAttribute[]{msToNext, dtmf}, "Produces the specified DTMF tone for the given time in milliseconds. When the milliseconds is set to 0 or omitted the tone will continue until it is stoped by setting the DTMF code to off.", Contitionals.none, Contitionals.none, Contitionals.isTimeCritical),
    ...
    clearTimer(false, false, new FeatureAttribute[]{listenerId}, "Clears the start time and terminates any pending events of any persistent timer matching listenerId (from the local storage data). After this a subsequent startTimer can be used to assign a new start time with this listenerId."),
    ...
    addMediaTrigger(true, false, new FeatureAttribute[]{msToNext, mediaId}, "Adds a media recording or playback event that will trigger when the media first passes the provided milliseconds value. The timing of this event will have a resolution not less than the length of the recording buffer."),
    addRecorderDtmfTrigger(true, false, new FeatureAttribute[]{dtmf}, "Adds a web recorder event that will trigger when the the provided DTMF tone is detected in the recorded audio stream. Only one trigger can be assigned per DTMF code."),
    ...
    playMedia(false, new FeatureAttribute[]{mediaId}, "Triggers play on any media with a mediaId that matches the provided regex string.", Contitionals.none, Contitionals.none, Contitionals.isTimeCritical),
    ...
    groupFullError(false, new FeatureAttribute[]{}, "When the specified group already has enough members new members cannot assign to it and the contents of this element will be triggered. It is recomended that the paricipant is instructed here to request a new group which can be assigned in the URL. Please note that a group and its members are rememberd on the server by design and reloading the page or clearing the browser storage will have no effect on group status.", Contitionals.any, Contitionals.groupNetwork),
