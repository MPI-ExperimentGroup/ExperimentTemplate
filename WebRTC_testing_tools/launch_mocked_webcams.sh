...
if [ ! -e "videoA.y4m" ]; then
    ...
    ffmpeg -f lavfi -i "sine=frequency=1000:duration=30" -af "volume=0.5" audio1.wav
    ffmpeg -f lavfi -i "sine=frequency=800:duration=30" -af "volume=0.5" audio2.wav
    ffmpeg -f lavfi -i "sine=frequency=600:duration=30" -af "volume=0.5" audio3.wav
fi
