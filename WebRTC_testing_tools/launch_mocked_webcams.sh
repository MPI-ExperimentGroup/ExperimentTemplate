...
if [ ! -e "videoA.y4m" ]; then
    ffmpeg -f lavfi -i color=c=red:s=320x240:d=30:sine=frequency=1000:duration=1 -af "volume=0.5" -vf "drawtext=fontsize=48:fontcolor=white:font=Arial:text='WebCam A':x=20:y=20, drawtext=fontsize=28:fontcolor=white:text='%{pts\:hms}':x=20:y=100" -pix_fmt yuv420p videoA.y4m
    ffmpeg -f lavfi -i color=c=green:s=320x240:d=30:sine=frequency=800:duration=1 -af "volume=0.5" -vf "drawtext=fontsize=48:fontcolor=white:font=Arial:text='WebCam B':x=20:y=20, drawtext=fontsize=28:fontcolor=white:text='%{pts\:hms}':x=20:y=100" -pix_fmt yuv420p videoB.y4m
    ffmpeg -f lavfi -i color=c=blue:s=320x240:d=30sine=frequency=600:duration=1 -af "volume=0.5" -vf "drawtext=fontsize=48:fontcolor=white:font=Arial:text='WebCam C':x=20:y=20, drawtext=fontsize=28:fontcolor=white:text='%{pts\:hms}':x=20:y=100" -pix_fmt yuv420p videoC.y4m
